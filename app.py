Je vous présente mes excuses, il est possible que la modification n'ait pas été visible ou pas assez claire à cause de la similarité du code. 

Pour qu'il n'y ait aucun doute, j'ai explicitement ajouté les mentions **[NOUVEAU]** dans le menu déroulant des types de produits. Désormais, vous verrez clairement :
* `"Fenêtre avec Imposte [NOUVEAU]"`
* `"Porte avec Imposte [NOUVEAU]"`
* `"Imposte Seule [NOUVEAU]"`

De plus, l'interface s'adaptera automatiquement. Par exemple, si vous choisissez "Imposte Seule", le système vous demandera uniquement la hauteur de l'imposte et ignorera le champ vantail.

Voici le code complet et corrigé de `app.py` :
...
def view_devis():
    ...
    # Liste des produits avec les impostes ajoutées clairement
    categories = [
        "Fenêtre 1 vantail", 
        "Fenêtre 2 vantaux", 
        "Fenêtre Coulissante", 
    ...
    # Logique d'interface dynamique pour l'imposte
    is_imposte_seule = "Imposte Seule" in type_produit
    has_imposte = "avec Imposte" in type_produit
    
    col_d1, col_d2, col_d3 = st.columns(3)
    largeur = col_d1.number_input("Largeur Totale (mm)", 300, 6000, 1200, step=50)
    
    if is_imposte_seule:
        # Si c'est une imposte seule, on demande juste la hauteur de l'imposte
        hauteur = col_d2.number_input("Hauteur Imposte (mm)", 300, 3000, 500, step=50)
        hauteur_imposte = col_d3.number_input("Non applicable (0)", 0, 0, 0, step=1, help="Ce champ est désactivé pour une imposte seule.")
        h_imposte_val = 0
    elif has_imposte:
        # Si c'est un produit avec imposte, on demande le vantail + imposte
        hauteur = col_d2.number_input("Hauteur Vantail (mm)", 300, 6000, 1000, step=50)
        hauteur_imposte = col_d3.number_input("Hauteur Imposte (mm)", 100, 3000, 500, step=50)
        h_imposte_val = hauteur_imposte
    else:
        # Sinon, produit classique sans imposte
        hauteur = col_d2.number_input("Hauteur Vantail (mm)", 300, 6000, 1000, step=50)
